...
```{r, missingness-plot, fig.width=7.5, fig.height=11.5}
...
```{r, distribution-plot1, fig.height=5}
...
```{r, distribution-plot2, fig.height=5}
...
```{r,linearity-curvature-dist, fig.height=5}
...
```{r, partition plot, fig.width=7.5, fig.height=11.5}
plot_metric_partition(
          metric_summary = pm_diagnostic_metrics_group,
...
```{r, trajectories-plot1, fig.height=3}
# ungrouped version
plot_data_trajectories(pm_data)

```

```{r, trajectories-plot2, fig.height=5}
# grouped version
...
```{r, metric-trajectories-plot1, fig.height=3}
# ungrouped version
plot_data_trajectories(
        pm_data, 
        metric_summary = pm_diagnostic_metrics, 
...
In this Figure, countries highlighted based on the global threshold. The interactive version of the ungrouped dissimilarity plot available online via \url{https://oluwayomi-olaitan.github.io/rjournal-wdiexplorer-interactive-plots/} shows that hovering over each highlighted line displays the country name and the average dissimilarity distance value. This plot visually complements and reinforces the earlier findings from the `pm_variation` output generated by the `compute_variation()` function.

```{r, metric-trajectories-plot2, fig.height=5}
# grouped version
plot_data_trajectories(
        pm_data, 
        metric_summary = pm_diagnostic_metrics_group, 
