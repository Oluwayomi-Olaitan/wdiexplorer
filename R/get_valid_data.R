#' Extract valid data from the WDI data
#' Reports countries with no data point, countries with one data point, as well as years for which no data are available.
#'
#' @param wdi_data A data frame of the indicator data generated by \code{\link{get_wdi_data}}
#' @param index An optional character string specifying the indicator code
#' Defaults to `NULL`
#' @param verbose Logical, if TRUE, the message about countries and years will one or no data point is printed. If FALSE, it is silenced. Default to TRUE
#'
#' @returns A tibble with the valid data for the provided WDI indicator data set and a detailed report of missing entries.
#' @export
#'
#' @examples
#' get_valid_data(pm_data, verbose = TRUE)
get_valid_data <- function(wdi_data, index = NULL, verbose = TRUE) {
  # Identify the name of the variable in the wdi data that contains the country-year value as index
  if(is.null(index)) {
    index_var <- attr(wdi_data, "index_var")
    index = index_var[1]
  }

  # Identify years with no data points
  invalid_years <- wdi_data |>
    dplyr::group_by(dplyr::across(tidyselect::all_of("year"))) |>
    dplyr::summarise(n = sum(!is.na(.data[[index]])), .groups = "drop") |>
    dplyr::filter(.data$n == 0) |>
    dplyr::pull(.data$year)

  #return message about missing years
  year_message <- if(length(invalid_years) > 0) {
    paste("The", length(invalid_years),
          "year(s) listed below had no available data and were excluded:\n",
          paste(invalid_years, collapse = ", \t"))
  } else{
    "All years have at least one valid data point"
  }

  # Identify country with no data points
  invalid_countries <- wdi_data |>
    dplyr::group_by(dplyr::across(tidyselect::all_of("country"))) |>
    dplyr::summarise(n = sum(!is.na(.data[[index]]))) |>
    dplyr::filter(.data$n == 0) |>
    dplyr::pull(.data$country)

  # Identify country with one data point
  singleton_countries <- wdi_data |>
    dplyr::group_by(dplyr::across(tidyselect::all_of("country"))) |>
    dplyr::summarise(n = sum(!is.na(.data[[index]]))) |>
    dplyr::filter(.data$n == 1) |>
    dplyr::pull(.data$country)

  # return message about missing countries
  country_message <-if (length(invalid_countries) > 0 && length(singleton_countries) > 0){
    # both invalid countries or singleton countries
    paste("The", length(invalid_countries),
          "countries listed below have no available data and were excluded:\n",
          paste(invalid_countries, collapse = "\n- "),
          "\n\n",
          "The", length(singleton_countries),
          "countries listed below have one available data point and were excluded:\n",
          paste(singleton_countries, collapse = "\n- ")
    )
  } else if (length(singleton_countries) > 0) {
    # the singleton countries
    paste("The", length(singleton_countries),
          "countries listed below have one available data point and were excluded:\n",
          paste(singleton_countries, collapse = "\n- "))
  } else if (length(invalid_countries) > 0) {
    # the invalid countries
    paste("The", length(invalid_countries),
          "countries listed below have no available data and were excluded:\n",
          paste(invalid_countries, collapse = "\n- "))
  } else {
    "All countries have at least one valid data point"
  }

  # filter valid countries and years where actual data were collected, ignoring the WDI defaults
  valid_data <- wdi_data |>
    dplyr::arrange(dplyr::across(tidyselect::all_of("year"))) |> # arrange year in ascending order
    dplyr::group_by(dplyr::across(tidyselect::all_of("year"))) |>
    dplyr::filter(sum(!is.na(.data[[index]])) > 0) |> #Ensure a year level has at least one valid point
    dplyr::ungroup() |>
    dplyr::group_by(dplyr::across(tidyselect::all_of("country"))) |>
    dplyr::filter(sum(!is.na(.data[[index]])) > 1) |> #Ensure a country level has more than one valid point
    dplyr::ungroup()

  # print the messages
  if(verbose){
    message(country_message, "\n\n", year_message)
  }

  return(valid_data)
}
